...
Each account is a submenu: the row shows the profile, a battery-drain gauge, and the session (5h) percentage with its live reset countdown, and hovering reveals the detail — email, account number, per-window resets, the session window's start/reset schedule, per-model limits, and any overage. Per-account, that submenu also sets which percentages the title shows (*Show in menu-bar title*) and a custom auto-switch-away limit (*Auto-swap away at*) so a secondary account can be leaned on up to its own threshold before the engine moves off it. By default the title is **stacked** — one small-font line per managed account, the active one marked with a **●** — so you see every account's quota at a glance. *Settings* toggles that (*Stack accounts (multi-line)*), a battery gauge (*Battery gauge in title*), per-model / scoped limits (*Show model limits in title*), and the ⇄ glyph (*Show swap icon*).

You can also set these from the terminal (handy for scripting or a second machine), e.g. to show only 5h for one account and hide its weekly:

```bash
cswap ui stacked on|off
cswap ui scoped on|off                 # per-model / scoped ("cowork") limits in the title
cswap ui title <num|email> 5h          # that account's title: 5h only (default|off|5h|7d|both)
```
